...
## Choose a task-fit metric

Use an Oven only when it makes the user-approved outcome clearer:

| Work | Typical evidence |
| --- | --- |
| UI, CSS, layout, or rendering | Visual Parity screenshots with explained pixel drift |
| Reference/candidate, migration, or compatibility work | Differential Testing |
| Latency, frame pacing, throughput, or budget work | Performance Tracing against a real baseline |
| Model, geometry, or animation work | Model Lab against its declared model contract |
...
Ovens carry an `id@version` identity, and a shipped Oven can be pinned per project with `burnlist oven adopt <id>`.
...
**The project owns and runs the adapter—Burnlist never does.** `burnlist oven set` validates and snapshots supplied JSON, `burnlist oven bind` only records where producer-managed data lives, and the dashboard only reads it. Re-run the adapter and `set` after each batch, or bind a producer-owned path refreshed on a schedule or in CI—a stale adapter is a stale Oven.
...
2. Open the [canonical dashboard URL](/dashboard#dashboard-urls).
3. Inspect desktop and narrow/mobile layouts in a browser.
4. Exercise relevant empty, waiting, blocked, stale, error, and completed states.
...
Compilation and DOM tests are necessary but not sufficient visual evidence.
The dashboard remains read-only and the Oven remains declarative.
