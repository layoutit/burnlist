...
test("dashboard Delta chart stays source-backed while the log reports frame advances", () => {
  ...
  assert.match(root.innerHTML, /class="fail">1,000<\/span><span class="separator">·<\/span><span class="pass">238 \(23\.8%\)<\/span>/u);
  assert.match(root.innerHTML, /class="driving-parity-kpi-heading differential-scenario-heading">Scenario</u);
  assert.match(root.innerHTML, /class="driving-parity-kpi-gauge driving-parity-kpi-scenario-icon"/u);
...
`);
  ...
  assert.equal((await fetch(`${baseUrl}ovens/differential-testing/view`)).status, 404);
