...
Burnlist Progress is queue state: active checklist items burn down and completed ledger entries move progress toward `100%`. Checklist is the default Oven for that behavior. Target is source-backed metric-to-goal state: measured values move toward explicit targets at a declared active gate. Keep Target viewers separate from this Progress dashboard unless a narrow link or URL contract is explicitly required.
