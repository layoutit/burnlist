...
test("B7 progress evidence is an isolated exact family with glyphcss provenance", async () => {
  const evidence = JSON.parse(await readFile(join(root, "tui/src/oven-runtime/terminal-progress-evidence-index.json"), "utf8")), index = JSON.parse(await readFile(join(root, "dashboard/src/generated/terminal-progress-frames/index.json"), "utf8"));
  assert.equal(evidence.records.length, 37); assert.equal(index.entries.length, 26); assert.equal(evidence.records.filter((row) => row.target.startsWith("atom:")).length, 17);
  assert.deepEqual(new Set(index.entries.filter((row) => row.targetKind).map((row) => row.targetKind)), new Set(["kpi-strip", "kpi-item", "progress-donut", "burn-donut", "waffle-metric", "progress-value"]));
  for (const entry of index.entries) { const text = await readFile(join(root, "dashboard/src/generated/terminal-progress-frames", entry.path), "utf8"), frame = JSON.parse(text); assert.ok(text.split("\n").length - 1 <= 400); for (const name of ["@opentui/core", "@opentui/react", "glyphcss", "@glyphcss/core", "@glyphcss/effects"]) assert.ok(frame.renderer.packages[name].integrity); }
});

test("B7 audit rejects overreach, stale hashes, swapped frames, missing provenance, and incomplete family", async () => {
  await withFixture(async (target) => { await replace(target, "tui/src/oven-runtime/capability-registry.ts", 'sourceFamilyId: "grammar:element:kpi-strip"', 'sourceFamilyId: "grammar:element:log-table"'); await fails(target, /progress|family|missing|extra/u, ["scripts/audit-terminal-oven-parity.mjs", "--official-ovens"]); });
  await withFixture(async (target) => { await replace(target, "tui/src/oven-runtime/terminal-progress-evidence-index.json", '"artifactSha256":"', '"artifactSha256":"0'); await fails(target, /progress|hash|frame index/u, ["scripts/audit-terminal-oven-parity.mjs", "--official-ovens"]); });
  await withFixture(async (target) => { const path = join(target, "tui/src/oven-runtime/terminal-progress-evidence-index.json"), value = JSON.parse(await readFile(path, "utf8")), atom = value.records.find((row) => row.target.startsWith("atom:")), error = value.records.find((row) => row.frameId.endsWith(":required-error")); for (const key of ["frameId", "artifactPath", "artifactSha256"]) [atom[key], error[key]] = [error[key], atom[key]]; await writeFile(path, `${JSON.stringify(value)}\n`); await fails(target, /independently compiled|kind|trace|target/u, ["scripts/audit-terminal-oven-parity.mjs", "--official-ovens"]); });
  await withFixture(async (target) => { const path = join(target, "tui/src/oven-runtime/terminal-progress-evidence-index.json"), value = JSON.parse(await readFile(path, "utf8")), mapped = value.records.filter((row) => row.target.startsWith("atom:") && row.targetKind !== "kpi-strip"); for (const key of ["frameId", "artifactPath", "artifactSha256", "targetKind", "semanticSignature", "trace"]) [mapped[0][key], mapped[1][key]] = [mapped[1][key], mapped[0][key]]; await writeFile(path, `${JSON.stringify(value)}\n`); await fails(target, /kind|trace|target/u, ["scripts/audit-terminal-oven-parity.mjs", "--official-ovens"]); });
  await withFixture(async (target) => { const indexPath = join(target, "dashboard/src/generated/terminal-progress-frames/index.json"), index = JSON.parse(await readFile(indexPath, "utf8")), artifact = join(target, "dashboard/src/generated/terminal-progress-frames", index.entries[0].path), frame = JSON.parse(await readFile(artifact, "utf8")); delete frame.renderer.packages.glyphcss; const text = `${JSON.stringify(frame, null, 2)}\n`, digest = createHash("sha256").update(text).digest("hex"); await writeFile(artifact, text); index.entries[0].sha256 = digest; await writeFile(indexPath, `${JSON.stringify(index, null, 2)}\n`); const evidencePath = join(target, "tui/src/oven-runtime/terminal-progress-evidence-index.json"), evidence = JSON.parse(await readFile(evidencePath, "utf8")), record = evidence.records.find((row) => row.frameId === index.entries[0].id); record.artifactSha256 = digest; await writeFile(evidencePath, `${JSON.stringify(evidence, null, 2)}\n`); await fails(target, /provenance/u, ["scripts/audit-terminal-oven-parity.mjs", "--official-ovens"]); });
  await withFixture(async (target) => { await replace(target, "tui/src/oven-runtime/capability-registry.ts", 'fixtureIds: ["progress-components"]', 'fixtureIds: ["missing-progress-family"]'); await fails(target, /progress family|claims|registry requires/u, ["scripts/audit-terminal-oven-parity.mjs", "--official-ovens"]); });
});

test("B7 independently compiled trace rejects a consistent atom implementation record and frame-label swap", async () => withFixture(async (target) => {
  const evidencePath = join(target, "tui/src/oven-runtime/terminal-progress-evidence-index.json"), evidence = JSON.parse(await readFile(evidencePath, "utf8"));
  const progress = evidence.records.find((row) => row.target === "atom:grammar:element:progress-donut"), value = evidence.records.find((row) => row.target === "atom:grammar:element:progress-value");
  for (const key of ["fixture", "frameId", "artifactPath", "artifactSha256", "implementationExport", "targetKind", "semanticSignature", "trace"]) [progress[key], value[key]] = [value[key], progress[key]];
  await writeFile(evidencePath, `${JSON.stringify(evidence)}\n`);
  const indexPath = join(target, "dashboard/src/generated/terminal-progress-frames/index.json"), index = JSON.parse(await readFile(indexPath, "utf8"));
  const progressFrame = index.entries.find((row) => row.targetKind === "progress-donut"), valueFrame = index.entries.find((row) => row.targetKind === "progress-value");
  for (const key of ["targetKind", "semanticSignature", "trace"]) [progressFrame[key], valueFrame[key]] = [valueFrame[key], progressFrame[key]];
  await writeFile(indexPath, `${JSON.stringify(index)}\n`);
  await fails(target, /kind|trace|independently compiled target/u, ["scripts/audit-terminal-oven-parity.mjs", "--official-ovens"]);
}));
