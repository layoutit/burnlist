...
test("top-level and Oven help expose the validated use and set flow", () => {
  ...
  try {
    ...
    assert.match(top.stdout, /burnlist loop next <RunRef> \[--repo <path>\]/u);
    assert.match(top.stdout, /burnlist loop submit <RunRef> \(--outcome <complete\|approve\|reject\|escalate> \| --result <file>\) \[--repo <path>\]/u);
    assert.match(top.stdout, /burnlist loop claim <RunRef> \[--repo <path>\] \(recovery\/diagnostics\)/u);
...
test("nested Loop help snapshots every Stage 1 control", () => {
  ...
  try {
    ...
    for (const command of ["create", "next", "submit <RunRef> \\(--outcome <complete\\|approve\\|reject\\|escalate> \\| --result <file>\\)", "claim", "report <ClaimRef> \\(--outcome <complete\\|approve> \\| --result <file>\\)", "abandon <ClaimRef> --reason <host-cancelled|host-lost|expired>", "list", "pause|stop|complete", "status|inspect", "reconcile"]) {
