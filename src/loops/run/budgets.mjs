...
export function foldBudgets({ records, graph }) {
  ...
  const visits = {}, managedAttempts = new Set(records.filter((record) => record.value.type === "external-claim-bound")
    .map((record) => `${record.value.payload.claim?.nodeId}\0${record.value.payload.claim?.attempt}`)); let elapsedMilliseconds = 0, previous = records[0]?.value?.at;
  for (const record of records) {
    ...
    // A managed host claim is the agent-start boundary.  It replaces (rather
    // than supplements) node-started, so each accepted attempt is counted
    // exactly once regardless of its execution transport.
    if (type === "node-started" || type === "external-claim-bound") {
      if (type === "node-started" && managedAttempts.has(`${payload.nodeId}\0${payload.attempt}`)) continue;
      const node = nodes.get(type === "node-started" ? payload.nodeId : payload.claim?.nodeId);
      if (node?.kind === "agent") { counters.agentRuns += 1; if (node.mode === "task") counters.rounds += 1; }
      else if (type === "node-started" && node?.kind === "check") counters.checkRuns += 1;
    }
    ...
    if (type === "invocation-result" || type === "external-report-accepted") counters.outputBytes += payload.outputBytes;
  }
