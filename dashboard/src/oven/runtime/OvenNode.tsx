...
function runtimeSource(payload: unknown, item: unknown, source: unknown) {
  if (typeof source !== "string") return undefined;
  if (source === "@item") return item;
  if (source.startsWith("@item/")) return resolvePointer(item, source.slice(5));
  return resolvePointer(payload, source);
}
...
export function OvenNode({ node, ir, state, dispatch, item, path = "root" }: OvenNodeProps) {
  ...
  if (["checklist-current", "checklist-workspace", "checklist-tabs", "checklist-burn-panel", "checklist-ledger", "checklist-event-cards"].includes(node.kind)) return <ChecklistWidgetAdapter node={node} payload={state.payload} />;
  ...
  if (["ascii-block", "image-triptych", "field-list", "refresh-status", "verdict-header", "metric-tiles", "domain-note", "frame-card"].includes(node.kind)) return <WidgetAdapter node={node} ir={ir} state={state} dispatch={dispatch} item={item} />;
