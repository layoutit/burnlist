...
test("Loop Progress reuses the console ASCII topology and selected item", async () => {
  const payload = { raw: {
    selectedItemId: "H3",
    active: [
  ...
  try {
    ...
    for (const token of ["H3 · Review the terminal", "ACTIVE", "ASSIGNED LOOP · Review Loop", "MAKE", "VERIFY", "REVIEW", "DONE", "approve", "q:back"]) expect(frame).toContain(token);
