...
export function streamingDiffModel(node: TerminalNode, payload: JsonValue | undefined, selectedCard = 0, selectedFile = 0, expandedKey: string | null = null): StreamingDiffModel {
  ...
  const rawCards = source(node, payload), cards = (Array.isArray(rawCards) ? rawCards : []).map((raw: JsonValue) => {
    const card = asRow(raw), files = (Array.isArray(card.files) ? card.files : []).map((item) => {
...
function fileRows(file: StreamingDiffFile, expanded: boolean) { return 1 + (file.diff ? expanded ? Math.min(3, file.diff.split("\n").length) : 1 : 1); }
function fileWindow(files: readonly StreamingDiffFile[], selected: number, rows: number, expandedKey: string | null, revId: string) {
  const safe = Math.max(0, Math.min(selected, Math.max(0, files.length - 1))); let used = 0, start = safe;
  for (let index = safe; index >= 0; index -= 1) { const cost = fileRows(files[index]!, expandedKey === `${revId}:${files[index]!.path}`); if (used + cost > rows && index !== safe) break; used += cost; start = index; }
  return { items: files.slice(start, safe + 1), start };
}
...
export function TerminalStreamingDiff({ node, payload, width, height = 10, selectedCard = 0, selectedFile = 0, expandedKey = null }: { node: TerminalNode; payload?: JsonValue; width: number; height?: number; selectedCard?: number; selectedFile?: number; expandedKey?: string | null }) {
  ...
  const cardWindow = visibleWindow([...model.cards], model.selectedCard, 1), cards = cardWindow.items;
  return <box width={width} height={rows} flexDirection="column" overflow="hidden">
    {!cards.length ? <text fg={palette.dim}>{fitText("Waiting for diff cards.", width)}</text> : cards.map((card, cardIndex) => <box key={`${card.revId}:${cardIndex}`} flexDirection="column" overflow="hidden">
      ...
      {(() => { const files = fileWindow(card.files, cardWindow.start + cardIndex === model.selectedCard ? model.selectedFile : 0, Math.max(1, rows - 2 - (card.partialReason ? 1 : 0)), model.expandedKey, card.revId); return files.items.map((file, fileIndex) => { const absoluteFile = files.start + fileIndex, key = `${card.revId}:${file.path}`; return <FileLine key={key} file={file} width={width} expanded={model.expandedKey === key} selected={cardWindow.start + cardIndex === model.selectedCard && absoluteFile === model.selectedFile} />; }); })()}
    </box>)}
  </box>;
}
