...
test("official Streaming Diff IR renders bounded cards, metadata, and a footer at wide and narrow sizes", async () => {
  for (const [width, height] of [[78, 20], [34, 16]] as const) {
    ...
    flushSync(() => root.render(<TerminalOvenViewport result={result} footer="q:back" streaming={{ selectedCard: 0, selectedFile: 0, expandedKey: "a1b2:src/app.ts" }} />)); await setup.renderOnce();
    ...
    expect(frame).toContain("+new"); expect(frame).not.toContain("Diff content is unavailable.");
...
test("array-valued diff-card source follows canonical withholding semantics", () => {
  const model = streamingDiffModel(compiled.ir.root[1]!, payload, 0, 0, "a1b2:src/app.ts");
