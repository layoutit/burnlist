...
export function GlyphImage({ source, width, height }: { source: string | null; width: number; height: number }) {
  ...
  const nativeSource = useMemo(() => {
    if (!native || !source) return null;
    try { return nativeImageThumbnail(source, width, height); } catch { return null; }
  }, [height, native, source, width]);
  const result = useMemo(() => {
    if (nativeSource) return { frame: null, error: null };
  ...
  }, [height, nativeSource, source, width]);
  if (nativeSource) return <box width={width} height={height}>
    <image source={nativeSource} protocol="iterm" position="absolute" left={0} top={0} width={width} height={height} />
  </box>;
