...
test("all eight official IRs and legal scoped @item IR pass", async () => { for (const id of ["agent-monitor", "checklist", "differential-testing", "loop-progress", "model-lab", "performance-tracing", "streaming-diff", "visual-parity"]) expect(validateTerminalOvenIR(await official(id))).toEqual([]); const custom = await compile('<oven id="custom" version="1.0.0" contract="checklist-progress@1" theme="checklist"><collection id="rows" source="/rows" item-key="/id" paging="auto" page-size="25"><each><kpi-item id="row" value="@item/value"/></each></collection></oven>'); expect(custom.ok).toBeTrue(); expect(validateTerminalOvenIR(custom.ir)).toEqual([]); const unscoped: any = structuredClone(custom.ir); unscoped.root[0].attributes.source = "@item/nope"; expect(validateTerminalOvenIR(unscoped).map((x) => x.code)).toContain("IR_POINTER"); });
