...
function findNode(ir: any, kind: string) { let found: any; const walk = (node: any) => { if (!found && node.kind === kind) found = node; node.children.forEach(walk); }; ir.root.forEach(walk); return found; }
test("all nine official IRs and legal scoped @item IR pass", async () => { for (const id of ["agent-monitor", "checklist", "differential-testing", "loop-progress", "model-lab", "multi-monitor", "performance-tracing", "streaming-diff", "visual-parity"]) expect(validateTerminalOvenIR(await official(id))).toEqual([]); const custom = await compile('<oven id="custom" version="1.0.0" contract="checklist-progress@1" theme="checklist"><collection id="rows" source="/rows" item-key="/id" paging="auto" page-size="25"><each><kpi-item id="row" value="@item/value"/></each></collection></oven>'); expect(custom.ok).toBeTrue(); expect(validateTerminalOvenIR(custom.ir)).toEqual([]); const unscoped: any = structuredClone(custom.ir); unscoped.root[0].attributes.source = "@item/nope"; expect(validateTerminalOvenIR(unscoped).map((x) => x.code)).toContain("IR_POINTER"); });
...
test("IR rejects compiler-invalid attributes, children, and ids", async () => { const base: any = structuredClone(await official("checklist")), tabs = findNode(base, "checklist-tabs"); tabs.attributes = { unknownThing: "x" }; tabs.children = [{ kind: "text", attributes: { slot: "x", text: "x" }, bindings: {}, children: [], source: { offset: 0, line: 1, column: 1 } }]; tabs.attributes.id = "not valid!"; const codes = validateTerminalOvenIR(base).map((x) => x.code); expect(codes).toEqual(expect.arrayContaining(["IR_ATTRIBUTE", "IR_NODE", "IR_NODE_ID"])); });
