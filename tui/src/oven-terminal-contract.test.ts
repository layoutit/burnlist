...
test("IR rejects compiler-invalid attributes, children, and ids", async () => { const base: any = structuredClone(await official("checklist")), ledger = findNode(base, "checklist-ledger"); ledger.attributes = { unknownThing: "x" }; ledger.children = [{ kind: "text", attributes: { slot: "x", text: "x" }, bindings: {}, children: [], source: { offset: 0, line: 1, column: 1 } }]; ledger.attributes.id = "not valid!"; const codes = validateTerminalOvenIR(base).map((x) => x.code); expect(codes).toEqual(expect.arrayContaining(["IR_ATTRIBUTE", "IR_NODE", "IR_NODE_ID"])); });
